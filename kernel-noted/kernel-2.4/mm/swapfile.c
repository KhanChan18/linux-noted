...
void __swap_free(swp_entry_t entry, unsigned short count)
{
    ...
    // 任何页面交换设备或者文件中的0页面是不用于交换的，
    // 所以不能为0
	if (!entry.val)
		goto out;

    // 从给定的swap_entry中提取出设备号，校验是否是有效设备
	type = SWP_TYPE(entry);
	if (type >= nr_swapfiles)
		goto bad_nofile;
    
    // 此处取出的p是对应的swap_info_struct结构，代表设备
	p = & swap_info[type];
	if (!(p->flags & SWP_USED))
		goto bad_device;
    ...
    // 设备中的每个页面的引用计数也是有一定上限的；
    // 并且swap_map中储存的引用计数不能低于，传入的count
    // 因为传入的count代表想要释放这个页面的进程数量；
    //
    // 如果减去了count之后页面还在被引用，则判断此时页面
    // 的offset是否已经偏离了设备规定的页面序号范围；
    // 如果是，则调整；
    //
